 ...
 * @returns A score indicating the likelihood of being a temporary email (0-100) and an explanation
 */
export async function analyzeEmailWithAI(email: string): Promise<{
  ...
  try {
    ...
    // Clean up the explanation to remove any formatting instructions
    explanation = explanation.replace(/\[.+?\]/g, '').trim();
    
    // Truncate explanation to a reasonable length for display
